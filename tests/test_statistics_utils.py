...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        # Valores esperados para media móvil con window=2
    ...
    def test_moving_average_raises_for_invalid_window(self):
        ...
        - Llamar a moving_average con window=0 (valor no positivo) y verificar que se lanza un ValueError 
        (usar self.assertRaises)
        - Llamar a moving_average con window mayor que la longitud del array y verificar que se lanza un 
        ValueError (usar self.assertRaises)
        """
        utils = StatisticsUtils()
        arr = [1, 2, 3]

        # window = 0 (no positivo)
        with self.assertRaises(ValueError):
            utils.moving_average(arr, window=0)

        # window mayor que la longitud del array
        with self.assertRaises(ValueError):
    ...
    def test_moving_average_only_accepts_1d_sequences(self):
        ...
        - Llamar a moving_average con esa secuencia y verificar que se lanza un ValueError indicando que solo 
        se aceptan secuencias 1D (usar self.assertRaises)
        """
        utils = StatisticsUtils()
        arr = [[1, 2], [3, 4]]
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        - Verificar que la media del resultado es aproximadamente 0 (usar self.assertAlmostEqual para un solo 
        valor numérico - unittest es suficiente)
        ...
        #Verificar media; aproximadamente 0
        self.assertAlmostEqual(np.mean(result), 0.0, places=7)

        # Verificar desviación estándar; aproximadamente 1
    ...
    def test_zscore_raises_for_zero_std(self):
        ...
        - Llamar a zscore con esa secuencia y verificar que se lanza un ValueError indicando que la desviación 
        estándar es cero (usar self.assertRaises)
        """
        utils = StatisticsUtils()
        arr = [5,5,5]
    ...
    def test_min_max_scale_raises_for_constant_values(self):
        ...
        - Llamar a min_max_scale con esa secuencia y verificar que se lanza un ValueError indicando que todos 
        los valores son iguales (usar self.assertRaises)
        """
        utils = StatisticsUtils()
        arr = [5,5,5]
